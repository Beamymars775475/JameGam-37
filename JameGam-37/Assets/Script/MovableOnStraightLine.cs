...
public class MovableOnStraightLine : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && (mousePosition.x > initialPosition.x-maxToMove && mousePosition.x < initialPosition.x+maxToMove))
        {
        ...
        if (isDragging && (mousePosition.x > initialPosition.x-maxToMove && mousePosition.x < initialPosition.x+maxToMove))
        {
            rb.isKinematic = true;


            transform.position = new Vector3(mousePosition.x, gameObject.transform.position.y, 0);
